...
const CaseRegistrationReview: React.FC = () => {
  ...
  const handleStatusUpdate = async (id: number, status: string) => {
    try {
      ...
      await registrationService.updateCaseRegistrationStatus(id, status);
      ...
      alert(`個案報名已${status === 'Approved' ? '批准' : '拒絕'}`);
    } catch (err) {
